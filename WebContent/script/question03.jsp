...
$(function () {
	   ...
	   $("td").on("click", function() {
	      var menu = $(this).find("p").text().trim();
	      var price = $(this).find("code").text().trim();
	      
	      var flag = false;

	      for (var i = 0; i < orderList.length; i++) {
	         if (orderList[i].name == menu) {
	      ...
	      if (!flag) {
	         orderList.push({name: menu, price: parseInt(price), quantity: 1});
	      } else {
	   ...
	   function updateTable() {
	      ...
	      for (var i = 0; i < orderList.length; i++) {
	         var totalPrice = orderList[i].price * orderList[i].quantity;
	         orderMenu += "<tr><td>" + orderList[i].name + "</td><td>" + orderList[i].quantity + "</td><td>" + totalPrice + "</td></tr>";
	         orderPrice += totalPrice;
	      }

	      $("#order-table").html(orderMenu);
	      $("#total").text(orderPrice + " 원");
	   }
	   
	   $("#orderBtn").on("click", function () {
	      location.href = "question03_process.jsp";
	   });
	});
</script>
</html>
